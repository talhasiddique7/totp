...
export function moveAccountUp(id) {
    ...
    [_accounts[index - 1], _accounts[index]] = [_accounts[index], _accounts[index - 1]];
    _accounts.forEach((a, i) => a.order = i);
    saveAccounts();
}
...
export function moveAccountDown(id) {
    loadAccounts();
    const index = _accounts.findIndex(a => a.id === id);
    if (index === -1 || index >= _accounts.length - 1) return;

    [_accounts[index], _accounts[index + 1]] = [_accounts[index + 1], _accounts[index]];
    _accounts.forEach((a, i) => a.order = i);
    saveAccounts();
}
