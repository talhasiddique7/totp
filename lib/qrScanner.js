...
function runSubprocess(argv, timeoutMs = 15000) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const exitCode = GLib.spawn_check_exit_status(exitStatus);

            if (exitCode !== 0 && (!stdout || stdout.toString().trim() === '')) {
                reject(new Error(stderr ? stderr.toString() : `Process exited with code ${exitCode}`));
                return;
            }
